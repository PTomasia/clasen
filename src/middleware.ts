...
// - PBKDF2 com 100k iterations: cada verificação custa ~100ms de CPU server-side,
//   o que naturalmente limita atacantes a ~10 tentativas/seg/instância.
// - Senha de 24 chars random (gerada via crypto.randomBytes(18).toString('base64'))
//   = ~144 bits de entropia. Mesmo com 1000 instâncias paralelas, brute force
//   é computacionalmente inviável (>10^28 segundos vs 10^17 da idade do universo).
// Rate limiting in-memory foi tentado mas não funciona em serverless multi-instance
// do Vercel (cada instância tem seu próprio Map). Pra rate limit real, usar
// Upstash KV num sprint futuro se virar multiusuário.
